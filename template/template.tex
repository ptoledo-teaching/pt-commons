...
\begin{itemize}
    ...
    \item \inlinecode{\textbackslash build\{auto\}} - Build number (advances when a tracked source changes)
...
\begin{center}
    \begin{tblr}{
        colspec = {L{0.38\columnwidth}X C{0.14\columnwidth}},
        ...
        Automatic Table Wrapping & Available & 1.0 \\
    \end{tblr}
\end{center}
...
\subsection{Portable Highlighted Code}

The same environment uses Minted when enabled and the plain fallback otherwise:
...
\begin{itemize}
    \item \inlinecode{\textbackslash corporation\{...\}} - Company name
...
The package tracks source revisions when
\inlinecode{\textbackslash build\{auto\}} is used. The main
\texttt{jobname.tex} file is tracked by default. Additional inputs can be
registered one at a time; for example:

\begin{center}
    \inlinecode{\textbackslash buildsource\{file.tex\}}
\end{center}

State is stored in \texttt{jobname.buildcount} or
\texttt{jobnameVERSION.buildcount}, and remains unchanged across ordinary
reruns.
...
\begin{itemize}
    ...
    \item \textbf{Date}: 2026/08/28
    \item \textbf{Author}: Pedro Toledo Correa
    \item \textbf{License}: LaTeX Project Public License 1.3c or later
\end{itemize}
